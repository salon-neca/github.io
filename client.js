...
//      if(editMode)
  ...
  function loadCache(isImport)
  {
    ...
    if(isImport)
    {
      lgg( 'OK.db2mem:'+ (d.length/1024).toFixed(2) +'KB' );

      importDB(d); 
      lgg( 'OK.import@loc2mem' );
    }
  }
  ...
  function loadServer()
  {
    ...
    $.ajax(
    {
      ...
      error:function(e, f)
      {
        ...
        {
          lgg( 'ERROR:comm. x2' );
          var cf= confirm('General comm. error, reload now?');
...
//          return;
      ...
      {//        lgg( 'heads='+ x.getAllResponseHeaders());
  ...
  function signServer()
  {
    ...
    $.ajax(
    {
      ...
      {
        lgg( 'FAIL@client:'+ f );
        $("#sgn4But").val('Server Awakening:'+ f +', try again!');
      },
      success:function(r, s, x)
      {
        ...
        {
          lgg( 'FAIL@server:'+ r );
          $("#sgn4But").val('Wrong password, try again!');
...
//    lgg( '>log.sys2srv' );
    ...
    $.ajax(
    {
      ...
      success:function(r)
      {
        if(r.substring(0,4) !== 'P@cl') {
          lgg( 'FAIL@server:'+ r ); return; }

        nBar.innerText+= ' [i]Logs save OK '+ r.substr(5);
        lgg( 'OK.log2srv:'+ (d.length/1024).toFixed(2) +'KB' );
      },
      complete:function() {
        lgg( '--- '+ (new Date().toISOString()).substr(11,8) +' ---' );
      }
    });
  }
...
// *** PART 1 - CACHE SAVE
    ...
    nBar.innerText= ' [i]Local store OK';

    lgg( 'OK.db2loc:'+ (d.length/1024).toFixed(2) +'KB' );
    if(cchOnly) return;


// *** PART 2 - SERVER SAVE
    if(!navigator.onLine) {
      lgg( 'FAIL:offline' ); return; }
    else
    if(!isLogged) {
      lgg( 'FAIL:no password' ); return; }
    ...
    $.ajax(
    {
      ...
      {//        lgg( 'heads='+ x.getAllResponseHeaders());
        ...
        nBar.innerText= ' [i]Server save OK '+ r.substr(5);
        lgg( 'OK.db2srv:'+ (d.length/1024).toFixed(2) +'KB' );
      },
      complete:function() {
        clgSave();
      }
  ...
  function wakeupServer()
  {
    ...
    $.ajax(
    {
      ...
      success:function(r, s, x)
      {
        if(r.substr(0,6) !== 'awoken')
          lgg( 'UNEXPECTED@server:'+ r );
        else
          lgg( 'OK.AWOKEN' );
      }
    });
...
//  $('#headbar').click(function() { var t= nBar.innerText; nBar.innerText= lastNotif; lastNotif= t; });
  ...
  { // star A.
    if(curTab === 3)
    {
      if($("#help").css('display') !== 'none') {
        $("#help").css({display:'none'}); nBar.innerText= '...'; return; }
...
// *** &&...
  ...
  function()
  {
    if(!confirm('Selected client and their session history will be '
      +'completely removed from the files. \n\n' + 'Are you sure?')) return;

    nBar.innerText= ' [i]Client Delete:';
  ...
  function()
  {
    ...
    if(fnd >= 0)
    {
      ...
      nBar.innerText= ' [i]Session deleted';
    }
    else
      nBar.innerText= ' [!]Session missing';
  });
...
//      resetEdit(1);
    ...
    else
    {
      if(!confirm('Last selected client "duplicate" will be removed '
        +'from the files and their session history added to the first '
...
//        resetEdit(1);
        ...
        nBar.innerText= ' [i]Duplicate client deleted';
      }
      else
        nBar.innerText= ' [!]Duplicate client missing';
  ...
  $("#sgn4But").click(function() { //>Log In<
    dbPass= $('#pasIn').val();
    $("#sgn4But").val('Please wait...');

    setTimeout(function() { signLocal(); }, 99);
  });
  ...
  { //>Grant Persistence<
    ...
    if(!navigator.storage.persist) {
      lgg( 'No navigator.storage.persist!' ); return; }

    navigator.storage.persisted().then(function(getP)
    {
      if(getP) lgg( 'Storage persistence already granted!' );
      else
      {
        navigator.storage.persist().then(function(setP)
        {
          if(setP)
          {
            lgg( 'Storage persistence is now granted!' );
            $('#gpc4But').val('Persistance Granted');
            $('#gpc4But').css({background:'none', color:'black', 'box-shadow':'none'});
          }
          else
          {
            lgg( 'Storage persistence denied, \n'
                                + 'try again after enabing notifications!' );
            if(!Notification)
              lgg( 'Notifications unavailable, try Chromium!' );
            else
  ...
  function cchInfo()
  {
    ...
    if(navigator.storage)
    {
      ...
      if(navigator.storage.estimate)
      {
        ...
        {
          lgg( 'Used:'+ (est.usage/1048576).toFixed(2) +'MB \n'
  ...
  function orphanSes()
  {
    var i, j, fnd, s= 'NONE', z= -1;

    for(i= 0; i < hiTab.length; i++)
    {
      if(hiTab[i][2] === z) continue;
  ...
  function orphanCli()
  {
    ...
    lgg( 'orphanClients:'+s );
    setTimeout(function() { orphanSes(); }, 99);
  }
